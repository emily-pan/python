...
class Citizen:
    ...
    def remove_subordinate(self, cid: int) -> None:
        ...
        for i in range(len(self._subordinates)):
            if self._subordinates[i].cid == cid:
                self._subordinates[i].set_superior(None)
    ...
    def get_all_subordinates(self) -> List[Citizen]:
        ...
        >>> c11.become_subordinate_to(c3)
        >>> result = c3.get_all_subordinates()
        >>> assert result == [c2, c4, c5, c1, c11]
        """
        # Note: This method must call itself recursively
        ...
        if self._subordinates == []:
            ...
            sub = merge([], self._subordinates)
            for s in self._subordinates:
                sub = merge(sub, s.get_all_subordinates())
            return sub
    ...
    def get_closest_common_superior(self, cid: int) -> Citizen:
        ...
        if self._superior is None:
            return self
        elif isinstance(self.get_citizen(cid), Citizen):
            return self
        elif isinstance(self._superior.get_citizen(cid), Citizen):
            return self._superior
        else:
    ...
    def get_district_name(self) -> str:
        ...
        if isinstance(self, DistrictLeader):
            ...
            if isinstance(self.get_superior(), DistrictLeader):
                return self.get_superior().get_district_name()
            else:
                name = ''
                if self._superior is not None:
                    name = self.get_superior().get_district_name()
            return name
...
class Society:
    ...
    def add_citizen(self, citizen: Citizen, superior_id: int = None) -> None:
        ...
        if superior_id is None:
            for s in citizen.get_direct_subordinates():
                citizen.remove_subordinate(s.cid)
    ...
    def change_citizen_type(self, cid: int,
        ...
        """
        old_citizen = self._head.get_citizen(cid)
        if isinstance(old_citizen, DistrictLeader):
            old_citizen.rename_district(None)
            new_citizen = Citizen(old_citizen.cid,
                                  old_citizen.manufacturer,
                                  old_citizen.model_year,
                                  old_citizen.job,
                                  old_citizen.rating)
        else:
            new_citizen = DistrictLeader(old_citizen.cid,
                                         old_citizen.manufacturer,
                                         old_citizen.model_year,
                                         old_citizen.job,
                                         old_citizen.rating,
        ...
        if superior:
            ...
            old_citizen.set_superior(None)
        else:
            self._head = new_citizen
        # subordinates
        subordinate = old_citizen.get_direct_subordinates()
        if subordinate:
            for s in subordinate:
                s.become_subordinate_to(new_citizen)
                old_citizen.remove_subordinate(s.cid)
        return new_citizen
    ...
    def _swap_up(self, citizen: Citizen) -> Citizen:
        ...
        old_superior = citizen.get_superior()
        new_citizen = citizen
        superior = old_superior.get_superior()
        old_superior.job, new_citizen.job = new_citizen.job, old_superior.job
        if isinstance(old_superior, DistrictLeader):
            new_citizen = self.change_citizen_type(
                citizen.cid, old_superior.get_district_name())
            old_superior = self.change_citizen_type(old_superior.cid)
        old_superior.remove_subordinate(citizen.cid)
        # set superior and subordinate
        old_subordinate = new_citizen.get_direct_subordinates()
        for s2 in old_superior.get_direct_subordinates():
            new_citizen.add_subordinate(s2)
            old_superior.remove_subordinate(s2.cid)
        for s1 in old_subordinate:
            old_superior.add_subordinate(s1)
            new_citizen.remove_subordinate(s1.cid)
        old_superior.become_subordinate_to(new_citizen)
        old_superior.remove_subordinate(new_citizen.cid)
        if superior:
            superior.remove_subordinate(old_superior.cid)
            superior.add_subordinate(new_citizen)
            new_citizen.set_superior(superior)
        else:
            self.set_head(new_citizen)
        old_superior.set_superior(new_citizen)
        return new_citizen
    ...
    def promote_citizen(self, cid: int) -> None:
        ...
        """
        citizen = self._head.get_citizen(cid)
        superior = citizen.get_superior()
        if superior:
            if superior.rating > citizen.rating:
                return
            elif isinstance(citizen, DistrictLeader):
                return
            else:
    ...
    def delete_citizen(self, cid: int) -> None:
        ...
        if self._head == old_citizen:
            subordinates = old_citizen.get_direct_subordinates()
            if subordinates:
                highest = old_citizen.get_highest_rated_subordinate()
                old_citizen.remove_subordinate(highest.cid)
                subordinates = old_citizen.get_direct_subordinates()
                for s in subordinates:
                    highest.add_subordinate(s)
                    old_citizen.remove_subordinate(s.cid)
                    s.set_superior(highest)
                self._head = highest
            else:
                self._head = None
        else:
            ...
            if subordinates:
                subordinates = old_citizen.get_direct_subordinates()
                for s in subordinates:
                    superior.add_subordinate(s)
                    old_citizen.remove_subordinate(s.cid)
                    s.set_superior(superior)
